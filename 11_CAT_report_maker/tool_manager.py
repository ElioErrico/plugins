...
def fast_reply(fast_reply: dict, cat):
    ...
    if command not in {"crea_tool_11", "cancella_tool_11"}:
        return fast_reply
...
def agent_allowed_tools(allowed_tools, cat):
    ...
    # cat.send_ws_message(f"Enabled tools from 11 CAT_report_maker: {str(enabled_tools)}", "chat")
    return enabled_tools
...
def agent_prompt_prefix(prefix, cat):
    enabled_tools = get_enabled_tools(cat)
    if "create_report_in_word" not in enabled_tools:
        return prefix

    settings = cat.mad_hatter.get_plugin().load_settings() or {}
    prompt_line = (settings.get("create_report_in_word_description") or "").strip()
    if not prompt_line:
        return prefix
