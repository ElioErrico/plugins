...
def fast_reply(fast_reply: dict, cat):
    ...
    if command not in {"crea_tool_13", "cancella_tool_13"}:
        return fast_reply
...
def agent_allowed_tools(allowed_tools, cat):
    ...
    # cat.send_ws_message(f"Enabled tools from 13 CAT_video_analyzer: {str(enabled_tools)}", "chat")
    return enabled_tools
...
def agent_prompt_prefix(prefix, cat):
    enabled_tools = get_enabled_tools(cat)
    if "video_analyzer" not in enabled_tools:
        return prefix

    settings = cat.mad_hatter.get_plugin().load_settings() or {}
    prompt_line = (settings.get("video_analyzer_description") or "").strip()
    if not prompt_line:
        return prefix
