...
def fast_reply(fast_reply: dict, cat):
    ...
    if command not in {"crea_tool_04", "cancella_tool_04"}:
        return fast_reply
...
def agent_allowed_tools(allowed_tools, cat):
    ...
    # cat.send_ws_message(f"Enabled tools from 04 CAT_Declarative_memory_agent_search: {str(enabled_tools)}", "chat")
    return enabled_tools

@hook(priority=4)
def agent_prompt_prefix(prefix, cat):
    enabled_tools = get_enabled_tools(cat)
    if "declarative_search" not in enabled_tools:
        return prefix
    ...
    prompt_line = (settings.get("declarative_search_description") or "").format(
        tag_label=tag_label
    ).strip()
    if not prompt_line:
        return prefix
